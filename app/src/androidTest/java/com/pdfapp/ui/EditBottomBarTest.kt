 ...
 * without horizontal scroll — the three tools are toggles, Save is a trailing
 * FAB, Undo is always visible, and a tap-to-jump page chip folds in the old
 * page-nav row. Tool settings open on demand.
 */
...
class EditBottomBarTest {
    ...
    @Test
    fun bottomBar_exposes_tools_page_chip_and_save() {
        ...
        composeRule.onNodeWithContentDescription("Tool settings").assertIsDisplayed()
        composeRule.onNodeWithContentDescription("Undo").assertIsDisplayed()
        composeRule.onNodeWithContentDescription("Save PDF").assertIsDisplayed()
        // The folded-in page-nav chip (Phase C.3).
        composeRule.onNodeWithText("1 / 3").assertIsDisplayed()
    }

    @Test
    fun bottomBar_toggles_tool_and_opens_settings_and_saves() {
        ...
        composeRule.onNodeWithContentDescription("Tool settings").performClick()
        assertTrue(settingsOpened)
        composeRule.onNodeWithContentDescription("Save PDF").performClick()
        assertTrue(saved)
    }
}
