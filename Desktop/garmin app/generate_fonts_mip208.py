"""
Generate Hebrew bitmap fonts for MIP round 208x208 devices (Forerunner 55).
Sized between Solar (176px) and MIP-260.
Uses NotoSansHebrew-Regular from Garmin's own font library.

Glance content area is 144x75 (narrower than fr255's 176x93):
  - Small 20 (LH 28): longest date line (k"t adar alef) = 104px, fits the
    ~103px slot right of the day letter; top line edge = cy-LH-1 = 8 >= 0.
  - Medium 26 (LH 36): LH(Small)+LH(Medium) = 64 <= 74 keeps the year line on.
Widget sizes are mip260's 24/32/42 scaled by 208/260 -> 20/26/34
(longest date at 34px = 171px, clears the 208px round edge mid-screen).
"""
...
out = r"C:\Users\yosef\Desktop\garmin app\resources-mip208\fonts"
print("Generating MIP-208 Hebrew fonts (NotoSansHebrew)...")
generate_fnt("HebrewSmall",  20, out)   # titles, labels
generate_fnt("HebrewMedium", 26, out)   # content
generate_fnt("HebrewLarge",  34, out)   # date display
# Glance-scoped fonts for the glance strip.
generate_fnt("HebrewGlanceSmall",  20, out)
generate_fnt("HebrewGlanceMedium", 26, out)
print("Done!")
